...
pub(crate) fn write_live_snapshot(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
    match app_type {
        ...
        AppType::Qwen => {
            // Qwen uses switch mode - write provider settings directly to ~/.qwen/settings.json
        ...
        AppType::Cline => {
            // Cline uses switch mode - patch only managed fields in ~/.cline/data/globalState.json
            use crate::cline_config::{ClineProviderConfig, write_cline_provider_config};
            
            // Convert settings_config to ClineProviderConfig
            let cline_config: ClineProviderConfig = 
                serde_json::from_value(provider.settings_config.clone())
                .map_err(|e| AppError::Config(format!("Invalid Cline provider config: {e}")))?;
            
            // Write using patch strategy (only updates managed fields)
            write_cline_provider_config(&cline_config)?;
            log::info!("Cline provider '{}' written to live config", provider.id);
        }
    }
    Ok(())
}
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        ...
        AppType::Cline => {
            use crate::cline_config::read_cline_provider_config;
            
            // Read only the managed fields from Cline global state
            let cline_config = read_cline_provider_config()?;
            
            // Convert to JSON Value using From trait to get the correct field names
            Ok(cline_config.into())
        }
    }
}
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    let settings_config = match app_type {
        ...
        AppType::Qwen => {
            ...
            if !settings_path.exists() {
                return Err(AppError::localized(
                    "qwen.live.missing",
                    "Qwen Code 配置文件不存在",
                    "Qwen Code settings file is missing",
                ));
            }
        ...
        AppType::Cline => {
            ...
            let global_state_path = get_cline_global_state_path()?;
            if !global_state_path.exists() {
                return Err(AppError::localized(
                    "cline.config.missing",
                    "Cline 配置文件不存在",
                    "Cline configuration file is missing",
                ));
            }

            // Read only the managed fields and convert to JSON using From trait
            let cline_config = read_cline_provider_config()?;
            cline_config.into()
        }
...
/// into the Claw Switch database. Each provider found will be added to the
