...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.claw-switch/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// 存储在本地 ~/.claw-switch/settings.json，不随数据库同步
...
export interface OpenClawCompactionConfig {
  mode?: string; // e.g. "default"
  maxHistoryShare?: number; // e.g. 0.6
  reserveTokensFloor?: number; // e.g. 40000
  memoryFlush?: {
    enabled?: boolean;
  };
  [key: string]: unknown;
...
// OpenClaw contextPruning 配置
export interface OpenClawContextPruningConfig {
  mode?: string; // e.g. "cache-ttl"
  [key: string]: unknown;
}
