...
pub async fn get_config_status(app: String) -> Result<ConfigStatus, String> {
    match AppType::from_str(&app).map_err(|e| e.to_string())? {
        ...
        AppType::Qwen => {
            let config_path = crate::qwen_config::get_qwen_settings_path();
            let exists = config_path.exists();
            let path = crate::qwen_config::get_qwen_config_dir()
                .to_string_lossy()
                .to_string();
        ...
        AppType::Cline => {
            let config_path = crate::cline_config::get_cline_settings_path();
            let exists = config_path.exists();
            let path = crate::cline_config::get_cline_config_dir()
                .to_string_lossy()
                .to_string();
...
pub async fn get_config_dir(app: String) -> Result<String, String> {
    let dir = match AppType::from_str(&app).map_err(|e| e.to_string())? {
        ...
        AppType::Qwen => crate::qwen_config::get_qwen_config_dir(),
        AppType::Cline => crate::cline_config::get_cline_config_dir(),
    };
...
/// 打开技能的本地目录（在系统文件管理器中）
/// 优先打开指定 app 的 provider 目录，若不存在则回退到 SSOT 目录
#[tauri::command]
pub async fn open_skill_dir(
    handle: AppHandle,
    directory: String,
    app: Option<String>,
    _app_state: State<'_, crate::store::AppState>,
) -> Result<bool, String> {
    use crate::app_config::AppType;
    use crate::services::skill::SkillService;

    // 尝试打开指定 provider 的目录
    let skill_path = if let Some(app_str) = app {
        let app_type: AppType = app_str.parse().map_err(|e: crate::error::AppError| e.to_string())?;
        let app_dir = SkillService::get_app_skills_dir(&app_type).map_err(|e| e.to_string())?;
        let path = app_dir.join(&directory);
        if path.exists() {
            path
        } else {
            // provider 目录不存在，回退到 SSOT
    ...
    if !skill_path.exists() {
        return Err(format!("技能目录不存在: {}", skill_path.display()));
    }
    handle
        .opener()
