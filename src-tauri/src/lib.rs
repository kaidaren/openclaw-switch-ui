...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("clawswitch://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.claw-switch/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/claw-switch.log）
            {
                ...
                let log_file_path = log_dir.join("claw-switch.log");
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("claw-switch".into()),
                            }),
                        ])
            ...
            let db_path = app_config_dir.join("claw-switch.db");
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.clawswitch.desktop/applications/claw-switch-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/claw-switch-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first clawswitch:// URL
                        }
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::check_app_live_config_exists,
            ...
            commands::check_qwen_config_consistency,
            commands::refresh_qwen_live_config,
            commands::test_qwen_connection,
            commands::test_provider_connection,
            commands::check_cline_config_consistency,
            commands::refresh_cline_live_config,
            ...
            commands::check_cli_env,
            // Provider terminal
            commands::open_provider_terminal,
            // CLI tool installer (backend-driven)
            commands::install_cli_tool,
            commands::cancel_cli_install,
            commands::open_terminal_for_install,
            commands::uninstall_cli_tool,
            commands::cancel_cli_uninstall,
            commands::open_terminal_for_uninstall,
            // CLI 工具文件系统监听器
            commands::start_cli_watcher,
            commands::stop_cli_watcher,
            ...
            commands::get_openclaw_provider_models,
            ...
            commands::get_openclaw_service_status,
            commands::get_openclaw_service_detail,
            commands::start_openclaw_service,
            commands::stop_openclaw_service,
            commands::restart_openclaw_service,
            commands::install_openclaw_gateway,
            commands::get_openclaw_gateway,
            commands::set_openclaw_gateway,
            commands::reload_openclaw_gateway,
            commands::run_openclaw_diagnostic,
            ...
            commands::clear_openclaw_channel_config,
            commands::check_openclaw_feishu_plugin,
            commands::install_openclaw_feishu_plugin,
            commands::check_openclaw_dingtalk_plugin,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 clawswitch://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older Claw Switch version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including claw-switch.db)\n\
            2) If you see “database version is newer”, please upgrade Claw Switch\n\
