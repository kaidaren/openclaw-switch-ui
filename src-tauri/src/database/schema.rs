...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    5 => {
                        log::info!("迁移数据库从 v5 到 v6（Qwen Skills 支持）");
    ...
    fn migrate_v2_to_v3(conn: &Connection) -> Result<(), AppError> {
        ...
        // 说明：v3 结构将 Skills 的 SSOT 迁移到 ~/.claw-switch/skills/，
    ...
    fn migrate_v5_to_v6(conn: &Connection) -> Result<(), AppError> {
        ...
        log::info!("v5 -> v6 迁移完成：已添加 Qwen Skills 支持");
        Ok(())
    }
