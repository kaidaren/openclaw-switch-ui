...
export function BasicFormFields({
  ...
  return (
    <>
      {/* Slot for additional fields before name */}
      {beforeNameSlot}

      {/* ── 基础信息区块 ── */}
      <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-4">
        <p className="text-xs font-semibold text-text-muted uppercase tracking-wider">{t("providerForm.basicInfo")}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Dialog open={iconDialogOpen} onOpenChange={setIconDialogOpen}>
                      <DialogTrigger asChild>
                        <button
                          type="button"
                          className="w-8 h-8 flex-shrink-0 rounded-md border border-border hover:border-primary transition-colors cursor-pointer bg-muted/30 hover:bg-muted/50 flex items-center justify-center"
                          title={
                            currentIcon
