...
impl SkillService {
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        Ok(match app {
            ...
            AppType::Cline => home.join(".cline").join("skills"),
        })
    }
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 Claw Switch 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "claw-switch".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 Claw switch 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "claw-switch".to_string()));
