...
impl ProxyService {
    ...
    async fn sync_live_to_provider(&self, app_type: &AppType) -> Result<(), String> {
        let live_config = match app_type {
            ...
            AppType::Qwen => {
                // Qwen doesn't support proxy features
                return Err("Qwen Code 不支持代理功能".to_string());
            }
            AppType::Cline => {
                // Cline doesn't support proxy features
                return Err("Cline 不支持代理功能".to_string());
            }
        };
    ...
    async fn sync_live_config_to_provider(
        ...
        match app_type {
            ...
            AppType::Qwen => {
                // Qwen doesn't support proxy features, skip silently
            }
            AppType::Cline => {
                // Cline doesn't support proxy features, skip silently
            }
        }
    ...
    fn write_live_config_for_app(&self, app_type: &AppType, config: &Value) -> Result<(), String> {
        match app_type {
            ...
            AppType::Qwen => {
                // Qwen doesn't support proxy features
                Err("Qwen Code 不支持代理功能".to_string())
            }
            AppType::Cline => {
                // Cline doesn't support proxy features
                Err("Cline 不支持代理功能".to_string())
            }
        }
    }
    ...
    pub fn detect_takeover_in_live_config_for_app(&self, app_type: &AppType) -> bool {
        match app_type {
            ...
            AppType::Qwen => {
                // Qwen doesn't support proxy takeover
                false
            }
            AppType::Cline => {
                // Cline doesn't support proxy takeover
                false
            }
        }
    }
