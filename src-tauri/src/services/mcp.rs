...
impl McpService {
    ...
    fn sync_server_to_app_no_config(server: &McpServer, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::Qwen => {
                // Qwen doesn't support MCP
                log::debug!("Qwen Code doesn't support MCP, skipping sync");
            }
            AppType::Cline => {
                // Cline doesn't support MCP yet
                log::debug!("Cline doesn't support MCP yet, skipping sync");
            }
        }
        Ok(())
    }
    ...
    fn remove_server_from_app(_state: &AppState, id: &str, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::Qwen => {
                // Qwen doesn't support MCP
                log::debug!("Qwen Code doesn't support MCP, skipping remove");
            }
            AppType::Cline => {
                // Cline doesn't support MCP yet
                log::debug!("Cline doesn't support MCP yet, skipping remove");
            }
        }
        Ok(())
    }
