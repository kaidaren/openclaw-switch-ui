...
fn schema_create_tables_include_pricing_model_columns() {
    ...
    let enabled_qwen = get_column_info(&conn, "skills", "enabled_qwen");
...
#[test]
fn schema_migration_v5_adds_enabled_qwen() {
    let conn = Connection::open_in_memory().expect("open memory db");
    conn.execute_batch(
        r#"
        CREATE TABLE proxy_config (app_type TEXT PRIMARY KEY);
        ...
        CREATE TABLE mcp_servers (
            ...
            enabled_claude INTEGER NOT NULL DEFAULT 0, enabled_codex INTEGER NOT NULL DEFAULT 0,
            enabled_gemini INTEGER NOT NULL DEFAULT 0, enabled_opencode INTEGER NOT NULL DEFAULT 0
        );
        CREATE TABLE skills (
    ...
    assert!(
        Database::has_column(&conn, "skills", "enabled_qwen").expect("check enabled_qwen"),
        "skills.enabled_qwen should exist after v5->v6 migration"
    );
    let enabled_qwen = get_column_info(&conn, "skills", "enabled_qwen");
    assert_eq!(enabled_qwen.r#type, "BOOLEAN");
    assert_eq!(enabled_qwen.notnull, 1);
