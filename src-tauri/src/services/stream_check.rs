...
impl StreamCheckService {
    ...
    async fn check_once(
        ...
        let result = match app_type {
            ...
            AppType::Qwen => {
                // Qwen doesn't support stream check yet
                return Err(AppError::localized(
                    "qwen_no_stream_check",
                    "Qwen Code 暂不支持健康检查",
                    "Qwen Code does not support health check yet",
                ));
            }
            AppType::Cline => {
                // Cline doesn't support stream check yet
                return Err(AppError::localized(
                    "cline_no_stream_check",
                    "Cline 暂不支持健康检查",
                    "Cline does not support health check yet",
                ));
            }
        };
    ...
    fn resolve_test_model(
        ...
        match app_type {
            ...
            AppType::Qwen => {
                // Qwen uses model.name in settings_config
                Self::extract_qwen_model(provider).unwrap_or_else(|| "qwen3-coder-plus".to_string())
            }
            AppType::Cline => {
                // Cline uses the same model format as Claude
    ...
    fn extract_qwen_model(provider: &Provider) -> Option<String> {
        provider
            .settings_config
            .get("model")
