...
const CLAW_SWITCH_SQL_EXPORT_HEADER: &str = "-- Claw Switch SQLite 导出";
...
impl Database {
    ...
    pub fn import_sql_string(&self, sql_raw: &str) -> Result<String, AppError> {
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_claw_switch_sql_export(sql_content)?;
    ...
    fn validate_claw_switch_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(CLAW_SWITCH_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 Claw Switch 导出的 SQL 备份文件。",
            "Only SQL backups exported by Claw Switch are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("claw-switch.db");
        if !db_path.exists() {
            return Ok(None);
        }
