...
//!
//! Handles importing provider configurations via clawswitch:// URLs.

use super::utils::{decode_base64_param, infer_homepage_from_endpoint};
use super::DeepLinkImportRequest;
use crate::error::AppError;
use crate::provider::{Provider, ProviderMeta, UsageScript};
use crate::services::provider::extract_qwen_base_url;
...
pub(crate) fn build_provider_from_request(
    ...
    let settings_config = match app_type {
        ...
        AppType::Cline => build_cline_settings(request),
    };
...
fn build_qwen_settings(request: &DeepLinkImportRequest) -> serde_json::Value {
    ...
    // Build Qwen provider config
    // Format: { modelProviders, env, security, model }
    let mut config = serde_json::Map::new();

    // Build modelProviders
    let mut model_providers = serde_json::Map::new();
    let mut openai_models = Vec::new();

    if let Some(model) = &request.model {
        let mut model_config = serde_json::Map::new();
        model_config.insert("id".to_string(), json!(model));
        model_config.insert("name".to_string(), json!(model));
        
        if !endpoint.is_empty() {
            model_config.insert("baseUrl".to_string(), json!(endpoint));
        } else {
            model_config.insert("baseUrl".to_string(), json!("https://dashscope.aliyuncs.com/compatible-mode/v1"));
        }
        
        model_config.insert("envKey".to_string(), json!("DASHSCOPE_API_KEY"));
        openai_models.push(json!(model_config));
    }

    model_providers.insert("openai".to_string(), json!(openai_models));
    config.insert("modelProviders".to_string(), json!(model_providers));

    // Build env
    let mut env = serde_json::Map::new();
    if let Some(api_key) = &request.api_key {
        env.insert("DASHSCOPE_API_KEY".to_string(), json!(api_key));
    } else {
        env.insert("DASHSCOPE_API_KEY".to_string(), json!(""));
    }
    ...
    auth.insert("selectedType".to_string(), json!("openai"));
    security.insert("auth".to_string(), json!(auth));
    config.insert("security".to_string(), json!(security));
    ...
    if let Some(model) = &request.model {
        ...
        model_config.insert("name".to_string(), json!("qwen3-coder-plus"));
    }
    config.insert("model".to_string(), json!(model_config));

    json!(config)
}

fn build_cline_settings(request: &DeepLinkImportRequest) -> serde_json::Value {
    let endpoint = get_primary_endpoint(request);

    // Build Cline provider config
    // Format: { apiKey, baseUrl, model, ... }
    let mut config = serde_json::Map::new();
...
pub fn parse_and_merge_config(
    ...
    match request.app.as_deref().unwrap_or("") {
        ...
        "qwen" => merge_qwen_config(&mut merged, &config_value)?,
...
/// Merge Qwen configuration from config file
fn merge_qwen_config(
    request: &mut DeepLinkImportRequest,
    config: &serde_json::Value,
) -> Result<(), AppError> {
    // Extract API key from env if not provided in URL
    if request.api_key.as_ref().is_none_or(|s| s.is_empty()) {
        if let Some(env) = config.get("env").and_then(|v| v.as_object()) {
            // Find first non-empty API key from env
            for value in env.values() {
                if let Some(key) = value.as_str().filter(|s| !s.is_empty()) {
    ...
    // Extract base URL from modelProviders if not provided in URL
    if request.endpoint.as_ref().is_none_or(|s| s.is_empty()) {
        let base_url = extract_qwen_base_url(config);
    ...
    if request.homepage.as_ref().is_none_or(|s| s.is_empty()) {
        if let Some(endpoint) = request.endpoint.as_ref().filter(|s| !s.is_empty()) {
            request.homepage = infer_homepage_from_endpoint(endpoint);
            // Default homepage for Qwen if inference fails
            if request.homepage.is_none() {
                request.homepage = Some("https://dashscope.aliyuncs.com".to_string());
            }
        }
    }

    // Extract model name if not provided in URL
    if request.model.is_none() {
        if let Some(model_name) = config
            ...
            .and_then(|v| v.as_str())
        {
            request.model = Some(model_name.to_string());
        }
    }
