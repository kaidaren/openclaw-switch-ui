...
impl ConfigService {
    ...
    fn sync_current_provider_for_app(
        ...
        match app_type {
            ...
            AppType::Qwen => {
                // Qwen uses switch mode, sync live config
                Self::sync_qwen_live(config, &current_id, &provider)?;
            }
            AppType::Cline => {
                // Cline uses switch mode, sync live config (similar to Claude)
                // For now, skip as Cline config format needs separate implementation
                log::debug!("Cline live sync not yet implemented");
            }
        }
    ...
    fn sync_qwen_live(
        ...
        use crate::services::provider::live::write_live_snapshot;

        // Write provider settings to live config
        write_live_snapshot(&AppType::Qwen, provider)
            .map_err(|e| AppError::Config(format!("Failed to write Qwen live config: {e}")))?;

        // Read back the actual written content and update to config
        let settings_path = crate::qwen_config::get_qwen_settings_path();
        let live_after = if settings_path.exists() {
            crate::config::read_json_file(&settings_path)?
        } else {
            serde_json::json!({})
        };
