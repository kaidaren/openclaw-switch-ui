  ...
  ({ initialApp = "claude" }, ref) => {
    ...
    return (
      <div className="px-6 flex flex-col h-[calc(100vh-8rem)] overflow-hidden bg-background/50">
        {/* 搜索框和筛选器 — 始终显示在顶部 */}
        <div className="pt-4 pb-3 flex flex-col gap-2 md:flex-row md:items-center shrink-0">
          ...
          <div className="w-full md:w-52">
            <Select value={filterRepo} onValueChange={setFilterRepo}>
              <SelectTrigger className="bg-card border shadow-sm text-foreground h-9">
                <SelectValue
              ...
              <SelectContent className="bg-card text-foreground shadow-lg max-h-64 min-w-[var(--radix-select-trigger-width)]">
                ...
                {repoOptions.map((repo) => (
                  <SelectItem
                    ...
                    title={repo}
                  >
                    <span className="truncate block max-w-[200px]">{repo}</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="pb-4">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-64 gap-3">
                ...
                <p className="text-sm text-muted-foreground/60">{t("common.loading", { defaultValue: "加载中..." })}</p>
              </div>
            ) : skills.length === 0 ? (
              ...
              <>
                {/* 结果计数 */}
                {(searchQuery || filterRepo !== "all" || filterStatus !== "all") && (
