...
const TOOL_NAMES = ["claude", "codex", "gemini", "opencode", "qwen"] as const;
...
const ONE_CLICK_INSTALL_COMMANDS = `
# Qwen Code
...
export function AboutSection({ isPortable }: AboutSectionProps) {
  ...
  const handleOpenReleaseNotes = useCallback(async () => {
    try {
      ...
      await settingsApi.openExternal(
        `https://github.com/heimanba/claw-switch/releases/tag/${displayVersion}`,
      );
    } catch (error) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <img src={appIcon} alt="Claw switch" className="h-5 w-5" />
              <h4 className="text-lg font-semibold text-foreground">
                Claw switch
              </h4>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="gap-1.5 bg-bg-primary/80">
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 px-1">
            {TOOL_NAMES.map((toolName, index) => {
              ...
              // Special case for OpenCode (capital C) and Qwen Code, others use capitalize
              const displayName =
                toolName === "opencode"
                  ? "OpenCode"
                  : toolName === "qwen"
      ...
      {!isWindows() && (
        ...
        >
          ...
          <div className="rounded-xl border border-border bg-gradient-to-br from-card/80 to-card/40 p-4 space-y-3 shadow-sm">
            ...
            <pre className="text-xs font-mono bg-bg-primary/80 px-3 py-2.5 rounded-lg border border-border/60 overflow-x-auto">
