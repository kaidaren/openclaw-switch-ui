...
const VALID_TOOLS: [&str; 6] = ["claude", "codex", "gemini", "opencode", "qwen", "openclaw"];
...
const MIN_NODE_MAJOR_DEFAULT: u32 = 18;

/// 检查 Node.js 主版本号是否满足要求
fn is_node_version_ok(version: &str, min_major: u32) -> bool {
    let parts: Vec<&str> = version.split('.').collect();
    if parts.is_empty() {
        return false;
    }
    match parts[0].parse::<u32>() {
        Ok(major) => major >= min_major,
        Err(_) => false,
    }
}

/// 检查 CLI 工具所需的运行环境（Node.js + CLI）
///
/// - has_node / node_version: 系统是否存在 Node.js 以及版本号（如 "22.11.0"）
...
pub async fn check_cli_env(tool: String, min_node_major: Option<u32>) -> Result<CliEnvStatus, String> {
    if !VALID_TOOLS.contains(&tool.as_str()) {
        return Err(format!("Invalid tool: {}", tool));
    }

    let min_major = min_node_major.unwrap_or(MIN_NODE_MAJOR_DEFAULT);
    // 与 CLI 工具检测保持一致：先用注入扩展 PATH 的 sh -c 尝试，失败则回退到扫描已知路径
    let (node_version, _node_error) = {
        let direct = try_get_version("node");
        if direct.0.is_some() {
            direct
        } else {
            scan_cli_version("node")
        }
    };
    let has_node = node_version.is_some();
    let node_version_ok = node_version
        .as_deref()
        .map(|v| is_node_version_ok(v, min_major))
        .unwrap_or(false);

    let cli_info = get_single_tool_version_impl(&tool, None, None).await;
    let has_cli = cli_info.version.is_some();
    let cli_version = cli_info.version;
...
async fn get_single_tool_version_impl(
    ...
    let latest_version = match tool {
        ...
        "qwen" => fetch_npm_latest_version(&client, "@qwen-code/qwen-code").await,
...
/// 同步检测指定 app 的 CLI 是否已安装。
/// 供其他模块（如 ProviderService）在同步上下文中调用。
/// 返回 true 表示已安装，false 表示未安装或无对应 CLI。
pub fn is_cli_installed(app_type: &AppType) -> bool {
    let tool = match app_type {
        ...
        AppType::Qwen => "qwen",
        AppType::Cline => return false, // Cline 无独立 CLI 工具名，视为未安装
    };
...
fn try_get_version_wsl(
    ...
    // 防御性断言：tool 只能是预定义的值
    debug_assert!(
        ["claude", "codex", "gemini", "opencode", "qwen", "openclaw"].contains(&tool),
...
fn wsl_distro_for_tool(tool: &str) -> Option<String> {
    let override_dir = match tool {
        ...
        "qwen" => crate::settings::get_qwen_override_dir(),
...
fn launch_macos_terminal(config_file: &std::path::Path) -> Result<(), String> {
    ...
    let temp_dir = std::env::temp_dir();
    let script_file = temp_dir.join(format!("claw_switch_launcher_{}.sh", std::process::id()));
...
fn launch_windows_terminal(
    ...
    let bat_file = temp_dir.join(format!("claw_switch_claude_{}.bat", std::process::id()));
