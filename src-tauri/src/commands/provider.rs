...
use crate::services::{
    EndpointLatency, ProviderService, ProviderSortUpdate, ProviderTestConnectionResult,
    QwenTestConnectionResult, SpeedtestService, SwitchResult,
};
use crate::store::AppState;
use std::str::FromStr;

/// 检查指定 app 的本地配置文件是否存在
/// 前端用此判断是否允许删除"使用中"的供应商
#[tauri::command]
pub fn check_app_live_config_exists(app: String) -> Result<bool, String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    Ok(ProviderService::app_live_config_exists(&app_type))
}
...
pub fn check_qwen_config_consistency(
    state: State<'_, AppState>,
    #[allow(non_snake_case)] providerId: String,
    #[allow(non_snake_case)] formConfig: serde_json::Value,
) -> Result<bool, String> {
    ProviderService::check_qwen_config_consistency(state.inner(), &providerId, &formConfig)
        .map_err(|e| e.to_string())
}
...
pub fn refresh_qwen_live_config(
    ...
    ProviderService::refresh_qwen_live_config(state.inner(), &providerId)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn test_qwen_connection(
    #[allow(non_snake_case)] selectedType: String,
    #[allow(non_snake_case)] baseUrl: String,
    #[allow(non_snake_case)] apiKey: String,
    #[allow(non_snake_case)] modelName: Option<String>,
) -> Result<QwenTestConnectionResult, String> {
    ProviderService::test_qwen_connection(&selectedType, &baseUrl, &apiKey, modelName.as_deref().unwrap_or(""))
        .await
        .map_err(|e| e.to_string())
}
...
pub fn check_cline_config_consistency(
    ...
    ProviderService::check_cline_config_consistency(state.inner(), &providerId, &formConfig)
        .map_err(|e| e.to_string())
}
...
pub fn refresh_cline_live_config(
    ...
    ProviderService::refresh_cline_live_config(state.inner(), &providerId)
        .map_err(|e| e.to_string())
}
...
/// 通用测试连接命令：适用于所有支持 OpenAI / Anthropic 协议的 Provider
#[tauri::command]
pub async fn test_provider_connection(
    ...
    #[allow(non_snake_case)] apiKey: String,
    #[allow(non_snake_case)] modelName: Option<String>,
) -> Result<ProviderTestConnectionResult, String> {
    ProviderService::test_provider_connection(
        &selectedType,
        &baseUrl,
        &apiKey,
        modelName.as_deref().unwrap_or(""),
    )
    .await
    .map_err(|e| e.to_string())
}
