...
impl McpApps {
    ...
    pub fn is_enabled_for(&self, app: &AppType) -> bool {
        match app {
            ...
            AppType::Qwen => false, // Qwen doesn't support MCP
            AppType::Cline => false, // Cline doesn't support MCP
        }
    }
    ...
    pub fn set_enabled_for(&mut self, app: &AppType, enabled: bool) {
        match app {
            ...
            AppType::Qwen => {} // Qwen doesn't support MCP, ignore
            AppType::Cline => {} // Cline doesn't support MCP, ignore
        }
    }
...
impl SkillApps {
    ...
    pub fn is_enabled_for(&self, app: &AppType) -> bool {
        match app {
            ...
            AppType::Cline => false,    // Cline doesn't support Skills
        }
    }
    ...
    pub fn set_enabled_for(&mut self, app: &AppType, enabled: bool) {
        match app {
            ...
            AppType::Cline => {}    // Cline doesn't support Skills, ignore
        }
    }
...
/// 未管理的 Skill（在应用目录中发现但未被 Claw switch 管理）
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub struct McpRoot {
    ...
    /// Qwen MCP 配置（Qwen 不支持 MCP，保留字段用于结构一致性）
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
    pub qwen: McpConfig,
    /// Cline MCP 配置（Cline 不支持 MCP，保留字段用于结构一致性）
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
    pub cline: McpConfig,
}
...
impl AppType {
    ...
    ///
    /// - Switch mode (false): Only the current provider is written to live config (Claude, Codex, Gemini, Qwen)
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            other => Err(AppError::localized(
                "unsupported_app",
                format!("不支持的应用标识: '{other}'。可选值: claude, codex, gemini, opencode, openclaw, qwen, cline。"),
                format!("Unsupported app id: '{other}'. Allowed: claude, codex, gemini, opencode, openclaw, qwen, cline."),
            )),
        }
    }
}
...
impl CommonConfigSnippets {
    ...
    pub fn get(&self, app: &AppType) -> Option<&String> {
        match app {
            ...
            AppType::Cline => self.cline.as_ref(),
        }
    }
    ...
    pub fn set(&mut self, app: &AppType, snippet: Option<String>) {
        match app {
            ...
            AppType::Cline => self.cline = snippet,
        }
    }
}
...
impl Default for MultiAppConfig {
    fn default() -> Self {
        ...
        apps.insert("cline".to_string(), ProviderManager::default());
...
impl MultiAppConfig {
    ...
    pub fn save(&self) -> Result<(), AppError> {
        let config_path = get_app_config_path();
        // 先备份旧版（若存在）到 ~/.claw-switch/config.json.bak，再写入新内容
        if config_path.exists() {
    ...
    pub fn mcp_for_mut(&mut self, app: &AppType) -> &mut McpConfig {
        match app {
            ...
            AppType::Cline => &mut self.mcp.cline,
        }
    }
    ...
    fn default_with_auto_import() -> Result<Self, AppError> {
        ...
        Self::auto_import_prompt_if_exists(&mut config, AppType::Cline)?;
    ...
    fn maybe_auto_import_prompts_for_existing_config(&mut self) -> Result<bool, AppError> {
        ...
        if !self.prompts.claude.prompts.is_empty()
            ...
            || !self.prompts.cline.prompts.is_empty()
        {
            return Ok(false);
        }
    ...
    fn auto_import_prompt_if_exists(config: &mut Self, app: AppType) -> Result<bool, AppError> {
        ...
        let prompts = match app {
            ...
            AppType::Cline => &mut config.prompts.cline.prompts,
        };
    ...
    pub fn migrate_mcp_to_unified(&mut self) -> Result<bool, AppError> {
        ...
        for app in [
            ...
            let old_servers = match app {
                ...
                AppType::Qwen => continue, // Qwen doesn't support MCP, skip
                AppType::Cline => continue, // Cline doesn't support MCP yet, skip
            };
        ...
        self.mcp.cline = McpConfig::default();
