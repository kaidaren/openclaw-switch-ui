...
pub fn ensure_test_home() -> &'static Path {
    ...
    HOME.get_or_init(|| {
        let base = std::env::temp_dir().join("claw-switch-test-home");
        if base.exists() {
        ...
        // 用 claw_switch_TEST_HOME 显式覆盖，以确保测试不会污染真实用户目录。
        std::env::set_var("claw_switch_TEST_HOME", &base);
...
pub fn reset_test_fs() {
    ...
    for sub in [".claude", ".codex", ".claw-switch", ".gemini"] {
