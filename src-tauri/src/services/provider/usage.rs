...
use super::extract_qwen_base_url;
...
fn extract_base_url_from_provider(provider: &crate::provider::Provider) -> Option<String> {
    // Check if this is a Qwen provider (has modelProviders structure)
    if provider.settings_config.get("modelProviders").is_some() {
    ...
    // Try Claude/Gemini environment variable approach
    if let Some(env) = provider.settings_config.get("env") {
