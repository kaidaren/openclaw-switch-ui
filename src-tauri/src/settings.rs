...
/// 应用设置结构
///
/// 存储设备级别设置，保存在本地 `~/.claw-switch/settings.json`，不随数据库同步。
/// 这确保了云同步场景下多设备可以独立运作。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppSettings {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub cline_config_dir: Option<String>,
    ...
    /// 当前 Cline 供应商 ID（本地存储，优先于数据库 is_current）
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_cline: Option<String>,
...
pub fn get_cline_override_dir() -> Option<PathBuf> {
    let settings = settings_store().read().ok()?;
    settings
        .cline_config_dir
        .as_ref()
...
pub fn get_current_provider(app_type: &AppType) -> Option<String> {
    ...
    match app_type {
        ...
        AppType::Cline => settings.current_provider_cline.clone(),
    }
}
...
pub fn set_current_provider(app_type: &AppType, id: Option<&str>) -> Result<(), AppError> {
    ...
    match app_type {
        ...
        AppType::Cline => settings.current_provider_cline = id.map(|s| s.to_string()),
    }
